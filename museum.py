...
def enrich(g, p, body, dialog):
    ...
    cover_credit = ('Фото: ' + e(cover_report['credit']) + (' · съёмка ' if cover_report.get('dateKind') == 'shooting' else ' · публикация ') + '.'.join(reversed(cover_report['publishedAt'].split('-')))) if cover_report else 'Фото: '+e(next((r['credit'] for r in g['PHOTO_SOURCES']['photos'] if r['asset']==(p['images'] or [''])[0]+'.webp'),'Портфолио компании'))
    ...
    body = opening + tail[:insert_at] + journal(g, p) + '<div class="section museum-gallery-heading"><p class="eyebrow">Фотографии объекта</p><h2>Фасад. В деталях.</h2><p>Галерея объекта. Для архивных снимков точная дата съёмки не указана.</p></div>' + gallery + tail[insert_at:]
