...
export async function verifyRelease9({page,base,shots}){
 ...
 await page.locator('.site-version').click();assert.match(await page.locator('.release-current').innerText(),/Текущая/);assert.equal(await page.locator('.release-number').first().innerText(),version);
