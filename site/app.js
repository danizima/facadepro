...
if($('.project-gallery[data-gallery]')){
 ...
 function display(next){index=(next+keys.length)%keys.length;stage.removeAttribute('srcset');stage.removeAttribute('sizes');stage.src=asset(keys[index]);const sources=JSON.parse(gallery.dataset.gallerySources||'{}');if(sources[keys[index]]&&!window.facadeAsset){stage.srcset=sources[keys[index]];stage.sizes='100vw';}large.src=stage.src;stage.alt=large.alt=gallery.dataset.title+`. Фотография ${index+1}`;$('.gallery-position').textContent=$('.lightbox-counter').textContent=`${index+1} / ${keys.length}`;thumbs.forEach((b,i)=>b.setAttribute('aria-pressed',String(i===index)));}
