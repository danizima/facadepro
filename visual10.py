...
def header(g,base,active):
    ...
    shortcuts=''.join(f'<a href="{base}{url}.html">{label} <span aria-hidden="true">↗</span></a>' for url,label in [('kit','Пакет подрядчика'),('portfolio','Портфолио PDF'),('solutions','Подбор решения'),('map','Карта объектов'),('photo-request','Отправить фото объекта')])
    phone=re.sub(r'[^+0-9]','',g['CFG']['phone'])
    return f'''<a class="skip-link" href="#main">Перейти к содержанию</a><header class="header"><div class="header-inner"><a href="{base}index.html" class="brand" aria-label="ФАСАД.PRO — на главную"><img src="{base}assets/logo.svg" alt="ФАСАД.PRO" width="180" height="49"></a><nav class="navigation" id="navigation" aria-label="Основная навигация"><div class="nav-primary">{primary}</div><div class="nav-extras"><p class="eyebrow">Быстрый доступ</p><div class="nav-shortcuts">{shortcuts}</div><a class="nav-phone" href="tel:{phone}">{e(g['CFG']['phone'])}</a><a data-contact-open class="nav-request button button-light" href="{base}request.html">Обсудить проект {g['ARROW']}</a></div></nav><a data-contact-open class="header-contact" href="{base}request.html">Обсудить проект {g['ARROW']}</a><button class="menu-toggle" type="button" aria-expanded="false" aria-controls="navigation" aria-label="Открыть меню"><span></span><span></span></button></div></header>'''
