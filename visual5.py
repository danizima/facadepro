...
def project(g, p, index):
    ...
    if p['images']:
        thumbs = ''.join(f'<button type="button" class="gallery-thumb js-only" data-image="{j}" aria-label="Фотография {j+1}" aria-pressed="{str(j==0).lower()}">{img(key,p["title"]+f". Фото {j+1}","../",sizes="80px")}</button>' for j,key in enumerate(p['images']))
